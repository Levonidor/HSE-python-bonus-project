    ...
    with open("./telegram/data.csv", "r", encoding="utf-8") as csv:
        ...
        for i, info in enumerate(csv):
            if i == 0:
                ...
                try:
                    database.loc[i + minus_index] = pd.Series(
                        tuple(info.split(",")), TARGET_COLUMNS
                    )
                except Exception:
                    try:
                        database.loc[i + minus_index] = pd.Series(
                        ...
                        minus_index -= 1
        database = dataframe_col_to_float(
            database, [DatabaseNames.OVERALL_ANALYZE, DatabaseNames.NOWADAY_PRICE]
        )
...
def make_buying_database(
    database: pd.DataFrame, relevant_parametr: float
) -> pd.DataFrame:
    buying_database = pd.DataFrame(columns=BUYER_COLUMNS)
    index = 0
    for i in range(len(database)):
        if database.loc[i][DatabaseNames.OVERALL_ANALYZE] >= relevant_parametr:
            buying_database.loc[index] = pd.Series(
                (
                    database.loc[i][DatabaseNames.NAME],
                    database.loc[i][DatabaseNames.TICKER],
                    database.loc[i][DatabaseNames.OVERALL_ANALYZE],
...
def add_recommendated_share(
    recommendated_shares: pd.DataFrame, share: pd.Series, amount: int
) -> pd.DataFrame:
    index = len(recommendated_shares)
    if (
        share[DatabaseNames.TICKER]
        not in recommendated_shares[RecommendNames.TICKER].tolist()
    ):
        recommendated_shares.loc[index] = pd.Series(
            (
                share[DatabaseNames.TICKER],
                amount,
                share[DatabaseNames.NOWADAY_PRICE],
                amount * share[DatabaseNames.NOWADAY_PRICE],
                share[DatabaseNames.NAME],
            ),
            RECOMMEND_COLUMNS,
        )
    else:
        for i in range(len(recommendated_shares)):
            if (
                recommendated_shares.loc[i][RecommendNames.TICKER]
                == share[DatabaseNames.TICKER]
            ):
                ind = i
                break
        recommendated_shares.loc[ind] = pd.Series(
            (
                share[DatabaseNames.TICKER],
                amount + recommendated_shares.loc[ind][RecommendNames.AMOUNT],
                share[DatabaseNames.NOWADAY_PRICE],
                (amount + recommendated_shares.loc[ind][RecommendNames.AMOUNT])
                * share[DatabaseNames.NOWADAY_PRICE],
                share[DatabaseNames.NAME],
    ...
    while True:
        ...
        for i in range(len(database)):
            if (
                ...
                >= 0
                and ((budget // 100) // (database.loc[i][DatabaseNames.NOWADAY_PRICE]))
                > 0
            ):
                add_recommendated_share(
                    recommendated_shares,
                    database.loc[i],
                    ((budget // 100) // (database.loc[i][DatabaseNames.NOWADAY_PRICE])),
                )
                now_budget -= (
                    (budget // 100)
                    // (database.loc[i][DatabaseNames.NOWADAY_PRICE])
                ...
                now_budget - database.loc[i][DatabaseNames.NOWADAY_PRICE] >= 0
                and database.loc[i][DatabaseNames.NOWADAY_PRICE] < budget / 100 * 30
            ):
                add_recommendated_share(recommendated_shares, database.loc[i], 1)
                now_budget -= database.loc[i][DatabaseNames.NOWADAY_PRICE]
    make_vizualization(recommendated_shares)
    recommendated_shares = dataframe_col_to_int(
        recommendated_shares, [RecommendNames.AMOUNT]
    )
...
def make_vizualization(recommendated_shares: pd.DataFrame) -> None:
    ...
    plt.pie(
        recommendated_shares[RecommendNames.TOTAL_PRICE],
        labels=recommendated_shares[RecommendNames.TICKER],
    )
...
async def start_handler(msg: Message, state: FSMContext):
    ...
    kb.adjust(5, 5)
    await msg.answer(
        "Выберите насколько рискованной стратегии вы хотите придерживаться: где 0 - это самые волатильные акции, а 1 - только самые стабильные акции."
    )
    await msg.answer(
...
async def start_handler(msg: Message, state: FSMContext):
    ...
    while not (0 <= float(vol) <= 1):
        await msg.answer(
            "Кажется что-то пошло не так, попробуйте снова выбрать значение в пределах [0, 1]"
        )
    ...
    await msg.answer(
        "Отлично! Теперь введи свой бюджет в рублях:",
...
async def start_handler(msg: Message, state: FSMContext):
    ...
    rub = rub.replace(" ", "")
    rub = rub.replace(",", ".")
    while 1000 > float(rub):
        await msg.answer(
            "Я могу составить портфель из активов менее чем на 1000 рублей, пожалуйста, введите значение от 1000"
        )
        await state.update_data()
    ...
    await msg.answer(
        "После проведенного анализа я составил портфель из девирсифицированных активов, основываясь на вашей желаемой стратегии"
    )
    await msg.answer(
        "Желаю удачи, буду ждать вас, когда решите обновить портфель, воспользуйтесь командой /new"
    )
    options.clear()
    await state.clear()
